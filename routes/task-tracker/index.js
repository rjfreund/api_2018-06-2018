...
dbSeeder.seedDatabase(schemaName)
.catch(function(e){
    console.error(e);
    process.exit(-1);
});

router.use('/tasks', authenticate, function(req, res, next){req.tableName = "tasks"; next(); }, require('./tasks'));

router.get('/', setSchema, function getTables(req, res, next){    	
    db.select()
...
router.get('/:tableName', function getRecords(req, res, next){    
    respond(res, dataManager.autoJoin(schemaName, req.params.tableName, req.query.filter, req.query.whereOp, req.query.orderBy, req.params.isNested, req.user));
});
...
router.get('/:tableName/:filter', function getFilteredRecords(req, res, next) {
    getJson(req.params.filter)
